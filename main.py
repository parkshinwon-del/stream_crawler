...
st.write("""이 서비스는 사람인과 고용 24 페이지의 검색어를 입력하여
...
#공고를 조회할 사이트를 선택
site_select = st.radio("크롤링할 사이트 선택", ['사람인','고용24'],
                       horizontal=True)
...
with st.expander("상세 검색 조건", expanded=True):
    ...
    with col2:
        if site_select == '사람인':
            ...
            #딕셔너리 loc_options의 '키'만 모아서 리스트로 

            #st.write(selected_location)
            #크롤링 진행할 때 사용하기 위해서 문자열 추출
            #selected_location으로, '서울', '경기', '부산'같은 지역명'추출
            ...
            #지역, 직무, 경력, 학력
            region = st.text_input('지역코드를 입력하세요', value='11000',
                                   help='지역 코드 알수없는 관계로 서울로 제한')
...
if crawling_clicked :
    ...
    #2-1. 검색어나 필수요가 누락된 경우 안내
    #검색어(search_text) 가 없다.(not)
    if not search_text:
        st.warning('검색어를 입력해주세요!')
    # 2-2. 클롤링 시행하는 동안 '기다려주세요'라는 내용 표시
    else:
